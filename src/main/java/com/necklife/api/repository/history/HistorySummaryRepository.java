...
public interface HistorySummaryRepository extends MongoRepository<HistorySummaryEntity, String> {
	...
	//	List<HistorySummaryEntity> findPointByMemberIdAndDate(String memberId, LocalDate start_At,
	// LocalDate today);

	// field가 localDate 타입이므로 isoDate 변환이 필요함
	@Query(
			value = "{ 'memberId': ?0, 'date': {'$gte': ?1, '$lte': ?2 } }",
			fields = "{'totalHistoryPoint': 1, 'date': 1}")
	List<HistorySummaryEntity> findPointByMemberIdAndDate(
			String memberId, LocalDate start_At, LocalDate end_At);
}
