...
public class UpdateStreakService {
    ...
    public void execute(MemberEntity member, HistorySummaryEntity historySummaryEntity) {
        ...
        if (optionalStreak.isPresent()) {
            ...
            // 목표 달성 스트릭 업데이트
            if (goalAchieved) {
                if (streak.getLastGoalStreakDate() != null && streak.getLastGoalStreakDate().plusDays(1).equals(activityDate)) {
            ...
            // 히스토리 기록 스트릭 업데이트
            if (historyRecorded) {
                if (streak.getLastHistoryStreakDate() != null && streak.getLastHistoryStreakDate().plusDays(1).equals(activityDate)) {
                    streak.setCurrentHistoryStreak(streak.getCurrentHistoryStreak() + 1);
                    streak.setLastHistoryStreakDate(activityDate);
                } else if (streak.getLastHistoryStreakDate() != null && (streak.getLastHistoryStreakDate().equals(activityDate) || streak.getLastHistoryStreakDate().isAfter(activityDate))) {
                    // 그대로 do nothing
                } else {
