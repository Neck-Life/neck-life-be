...
public class AddPaymentService {
    ...
    public PaymentEntity execute(MemberEntity member, LocalDateTime dateTime,Long months, Double won) {
        ...
        // 이전 결제가 있지만 만료 또는 추가 결제인 경우
           ...
           return paymentRepository.save(PaymentEntity.builder()
                    .member(member)
                    .updatedAt(dateTime)
                    .endAt(recentUpdate.get().getEndAt().isBefore(now) ? now.plusMonths(months) : recentUpdate.get().getEndAt().plusMonths(months))
                    .paymentWon(won)
                    .build());
