...
public class GetGoalsUseCase {
    ...
    public GoalResponse execute(String memberId) {
        ...
        Optional<GoalEntity> activeGoalForMemberAtDate = goalRepository.findActiveGoalForMemberAtDate(memberId, now);

        if (activeGoalForMemberAtDate.isPresent()) {
            GoalEntity goalEntity = activeGoalForMemberAtDate.get();
            return GoalResponse.builder()
                    .goals(goalEntity.getGoals().stream().map(goalDetail -> GoalResponse.GoalDetailResponseDTO.builder()
                            .order(goalDetail.getOrders())
                            .type(goalDetail.getType())
                            .description(goalDetail.getDescription())
